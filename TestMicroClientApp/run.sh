...
RUNNING=$(docker inspect --format="{{ .State.Running }}" TestMicroClientApp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    echo "Stopping... with title TestMicroClientApp"
    docker rm -f TestMicroClientApp
fi
